## ################################################
## Installing R-packages ReconstPoFD from GitHub
## ################################################
...
your_path <- "/home/dom/ownCloud/Kneip_Liebl_Reconstruction/Simulation_Submission_2"
...
Y_target_true_mat_LIST  <- vector("list", c(4+2+2))
U_target_true_mat_LIST  <- vector("list", c(4+2+2))
Y_target_mat_LIST       <- vector("list", c(4+2+2))
U_target_mat_LIST       <- vector("list", c(4+2+2))
Y_target_list_LIST      <- vector("list", c(4+2+2))
U_target_list_LIST      <- vector("list", c(4+2+2))
A_target_vec_LIST       <- vector("list", c(4+2+2))
B_target_vec_LIST       <- vector("list", c(4+2+2))
##
list_names <- c("DGP1_n50_m15","DGP1_n50_m30","DGP1_n100_m15","DGP1_n100_m30",
                "DGP2_n50","DGP2_n100",
                "DGP3_n50","DGP3_n100")
##
...
for(DGP in c('DGP1','DGP2','DGP3')){# DGP <- 'DGP1'
  ##
  if(DGP=="DGP1"){nRegGrid  <-  nRegGrid_vec[1]}else{nRegGrid  <-  nRegGrid_vec[2]}
  ##
  while(TRUE){
  ...
  if(DGP=="DGP1"){
    for(n in nn){
      for(m in mm){ # n <- nn[1]; m <- mm[1]
        if(m == mm[1]){slct <- sort(sample(1:mm[2], size = mm[1], replace = F))}
        ...
        for(r in 1:R){ # r <- 1
          Y_target_list_LIST[[paste0(DGP,"_n",n,"_m",m)]][[r]]      <- SimDat[['Y_list']][[r]][slct]
          U_target_list_LIST[[paste0(DGP,"_n",n,"_m",m)]][[r]]      <- SimDat[['U_list']][[r]][slct]
        }
        ##
...
for(DGP in c('DGP1','DGP2','DGP3')){
  ...
  for(n in nn){
    if(DGP=='DGP1'){m_seq <- mm}else{m_seq <- NA}
    for(m in m_seq){
...
for(n in c(50, 100)){
  for(m in c(15,  30)){
    ## n <- 50; m <- 15
    load(file = paste0("DGP1_n",n,"_m",m,"_simResults.RData"))
    ##
...
DGP1_df_LaTeX <- xtable(DGP1_df, digits=c(0, 0,0,0,0, 2,2,2,2))
...
for(DGP in c('DGP2', 'DGP3')){ # DGP <- "DGP2"
  for(n in c(50, 100)){
    ##
    load(file = paste0(DGP,"_n",n,"_simResults.RData"))
    #load(file = paste0("ARCHIV2/",DGP,"_n",n,"_simResults.RData"))
    ##
    col_slct         <- !is.na(BiasSq_vec)
    BiasSq_vec       <- BiasSq_vec[col_slct]
    Var_vec          <- Var_vec[col_slct]
    MSE_vec          <- BiasSq_vec + Var_vec 
    ##
    DGP2n3_df_0 <- rbind.data.frame(DGP2n3_df_0,
...
DGP2n3_df_LaTeX <- xtable(DGP2n3_df, digits=c(0,0,0,0,1,5,5,5))
